...
// Modified Objects
var floodLevel = floodLevel ?? Math.round((Math.random() * 100))
...
flood.goUpSpeed = 60000;
flood.maxLevel = 100; // Server - Do not Edit.
flood.bilgeDelay = 2500; // Server - Do not Edit.
...
flood.renderLoop = flood.renderLoop ?? undefined
flood.lastBilge = 0;
flood.logicLevel = 0;
flood.level = 0;
flood.info = flood.info ?? undefined

// HTML references!
flood.html = flood.html ?? {};
flood.html.flood = flood.html.flood ?? document.querySelector('#flood')
flood.html.menu = flood.html.menu ?? document.querySelector('#flood-menu')
flood.html.message = flood.html.message ?? document.querySelector('#flood-message')

flood.audio = flood.audio ?? {};
...
function renderWater() {
    ...
    var renderLevel = Number(flood.html.flood.style.top.replace("%", "")); // 0 is top
